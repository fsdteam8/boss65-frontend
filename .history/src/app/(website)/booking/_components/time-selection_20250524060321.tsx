...
export default function TimeSelection() {
  ...
  const dhakaTime = utcToZonedTime(new Date(), DHAKA_TIMEZONE)

  // Format selected date for API call (ensure it's in Dhaka timezone)
  const dateOnly = selectedDate ? format(utcToZonedTime(selectedDate, DHAKA_TIMEZONE), "yyyy-MM-dd") : null
  ...
  const isPastDate = (date: Date) => {
    ...
    const dhakaNow = utcToZonedTime(new Date(), DHAKA_TIMEZONE)
  ...
  const isTimeSlotPast = (timeSlot: TimeSlot) => {
    ...
    const selectedDhaka = utcToZonedTime(selectedDate, DHAKA_TIMEZONE)
    const nowDhaka = utcToZonedTime(new Date(), DHAKA_TIMEZONE)
          ...
          <div className="space-y-2">
            ...
            {timeSlots.filter((slot) => slot.available && !isTimeSlotPast(slot)).length === 0 && (
              <p className="text-center py-8 text-gray-500">
                {selectedDate && isToday(utcToZonedTime(selectedDate, DHAKA_TIMEZONE))
